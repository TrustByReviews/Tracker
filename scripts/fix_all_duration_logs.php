...
/**
 * Script para corregir todos los logs con duration_seconds = 0
 */
...
echo "🔧 CORRIGIENDO TODOS LOS LOGS CON DURACIÓN CERO\n";
echo "===============================================\n\n";

try {
    // 1. Encontrar todos los logs con duration_seconds = 0 o null
    ...
    foreach ($pauseLogs as $log) {
        if ($log->started_at && $log->paused_at) {
            ...
            if ($duration > 0) {
                ...
                echo "      - diff: " . $endTime->diffInSeconds($startTime, false) . "s\n\n";
            }
        } else {
    ...
    foreach ($finishLogs as $log) {
        if ($log->started_at && $log->finished_at) {
            $startTime = \Carbon\Carbon::parse($log->started_at);
            $endTime = \Carbon\Carbon::parse($log->finished_at);
            $duration = max(0, $endTime->diffInSeconds($startTime, false));
    ...
    echo "🎉 ¡Todos los logs corregidos!\n";
    echo "=============================\n";
